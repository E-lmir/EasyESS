...
namespace EasyESS.Document
{
    public class DocumentService
    {
        ...
        public void IdCLIRegistration(InstallationInfo info)
        {
            ...
            File.AppendAllText("C:/idCommands.txt", $"id add user \"DocServiceUser\" - p password = \"11111Z\"");
            File.AppendAllText("C:/idCommands.txt", $"id assign -u \"DocServiceUser\" -r \"service\"");
            File.AppendAllText("C:/idCommands.txt", $"id add resource \"Directum.Core.DocumentService\" -c \"{audiencePath}\"");
            executor.Execute($"cd {info.IdCLIServiceInfo.ServiceFolder}", $"id add user \"DocServiceUser\" - p password = \"11111Z\"", $"id assign -u \"DocServiceUser\" -r \"service\"", $"id add resource \"Directum.Core.DocumentService\" -c \"{audiencePath}\"");
        }
